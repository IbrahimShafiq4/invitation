...
function buildScienceFoodObjects() {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        const liq = new THREE.Mesh(new THREE.CylinderGeometry(.15, .19, .28, 12),
            new THREE.MeshStandardMaterial({
                color: new THREE.Color().setHSL(hue, .9, .65), transparent: true, opacity: .9,
    ...
    for (let i = 0; i < 7; i++) {
        ...
        const liq = new THREE.Mesh(new THREE.SphereGeometry(.18, 12, 8),
            new THREE.MeshStandardMaterial({
                color: new THREE.Color().setHSL(hue, .95, .6), transparent: true, opacity: .85,
    ...
    for (let i = 0; i < 8; i++) {
        ...
        const liq = new THREE.Mesh(new THREE.CylinderGeometry(.06, .06, .18, 10),
            new THREE.MeshStandardMaterial({
                color: new THREE.Color().setHSL(hue, .95, .6), transparent: true, opacity: .9,
    ...
    for (let i = 0; i < 15; i++) {
        ...
        const m = new THREE.Mesh(new THREE.BoxGeometry(.22 + Math.random() * .2, .22 + Math.random() * .2, .22 + Math.random() * .2),
            new THREE.MeshStandardMaterial({
                color: new THREE.Color().setHSL(h, .88, .65),
        ...
        m.userData = {
            fo: Math.random() * Math.PI * 2, fs: rnd(.35, .65), baseY: y,
...
gsap.utils.toArray('.detail-card,.venue-card,.map-wrapper,.wish-form-card').forEach((el, i) => {
